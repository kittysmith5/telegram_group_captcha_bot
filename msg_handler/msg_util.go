...
func sendCapture(update *api.Update, newMember api.User) (res string, sentMsg api.Message) {
    ...
    randomNum1 := rand.Int()%250 + 250
    randomNum2 := rand.Int()%250 + 250

    txt := int2Sticker(randomNum1) + "\t " + plusSticker + "\t " + int2Sticker(randomNum2)
    txt += "等于多少" + "\u2753"

    markA := strconv.Itoa(rand.Int()%500 + 500)
    markB := strconv.Itoa(rand.Int()%500 + 500)
    markC := strconv.Itoa(rand.Int()%500 + 500)
    markD := strconv.Itoa(rand.Int()%500 + 500)
    markE := strconv.Itoa(rand.Int()%500 + 500)

    res = strconv.Itoa(randomNum1 + randomNum2)
